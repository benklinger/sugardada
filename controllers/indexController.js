...
exports.resendPlan = async (req, res) => {
  try {
    ...
    const step2 = (user.hasIBAccount &&
      ...
      ? `<li><del>Sign up for a trading account</del></li>`
      : `<li><a href="${alink}">Sign up for a trading account</a></li>`;
    const subject = `${user.name}'s Updated Plan`;
    const html = `
<p>Thanks for updating ${user.name}'s investment plan!<br>
You can revisit or update it anytime using <a href="${shareableUrl}">this link</a>.</p>
	
<p>If you invest <strong>$${user.monthlyInvestment}/mo</strong> in <strong>${user.investmentTicker.toUpperCase()}</strong> until ${user.name} turns ${endAge}, your total value could grow to <strong>${estValue}</strong> — that’s an return of <strong>~${roiPct}%</strong>.</p>
<p>${tickerParagraph}</p>
...
`;
    ...
    console.error("Error resending plan:", error);
    return res.status(500).json({ error: 'Error resending plan' });
  }
};
